---
title: Skjemaer | Microsoft Docs
description: Bruk skjemaer til å samle og vise informasjon fra en datakilde.
services: ''
suite: powerapps
...
ms.locfileid: "30997672"
---
# <a name="understand-data-forms-in-microsoft-powerapps"></a>Slik bruker du dataskjemaer i Microsoft PowerApps
...
| --- | --- | --- |
| **Bla gjennom etter en post** |**[Galleri](controls/control-gallery.md)**-kontroll |Filtrer, sorter, søk og bla gjennom postene i en datakilde, og velg en bestemt post. Vis bare noen få felt fra hver post for å vise flere poster om gangen, selv på en liten skjerm. |
| **Vis detaljene for en post** |**[Visningsskjema](controls/control-form-detail.md)**-kontroll |For en enkelt post, kan du vise mange av, eller alle felt i posten. |
| **Redigere eller opprette en post** |**[Redigeringsskjema](controls/control-form-detail.md)**-kontroll |Oppdater ett eller flere felt i en enkelt post (eller opprett en post som starter med standardverdier), og lagre disse endringene tilbake til den underliggende datakilden. |

Sett hver kontroll i ulike skjermer for å gjøre det enklere å skille dem fra hverandre:
...
* [Registrer deg](../signup-for-powerapps.md) for PowerApps, [installer](http://aka.ms/powerappsinstall) det, åpne det, og logg deg deretter på ved å angi samme legitimasjon som du brukte til å registrere deg.
* Finn ut hvordan du kan [konfigurere en kontroll](add-configure-controls.md) i PowerApps.

## <a name="explore-a-generated-app"></a>Utforskning av en generert app
PowerApps kan automatisk generere en app, basert på en datakilde angir. Hver app inneholder tre skjermer med kontroller som er beskrevet tidligere og formler som knytter dem sammen. Kjør disse ferdiglagde appene, tilpass dem til dine bestemte mål eller undersøk hvordan de fungerer, slik at du kan lære om nyttige konsepter som gjelder for dine egne apper. Undersøk skjermer, kontroller og formler som kjører en generert app i avsnittene nedenfor.  
...
| --- | --- | --- |
| **BrowseGallery1** |Vis poster fra **Aktiva**-datakilden. |**[Items](controls/properties-core.md)**-egenskapen i galleriet er angitt til en formel som er basert på **Aktiva**-datakilden. |
...
> [!NOTE]
> I en generert app, er **[Items](controls/properties-core.md)** angitt til en betydelig mer komplisert formel som standard, slik at brukeren kan sortere og søke etter poster. Senere i dette emnet finner du ut hvordan du utvikler formelen; det er for øyeblikket nok med en enklere versjon.

Brukeren kan opprette en post ved å velge +-tegnet over galleriet, i stedet for å finne enn post som skal vises eller redigeres. Opprett denne effekten ved å legge til en **[Image](controls/control-image.md)**-kontroll som viser et +-symbol i den, og angi **[OnSelect](controls/properties-core.md)**-egenskapen til denne formelen:
...
I dette eksemplet er kontrollens **[Text](controls/properties-core.md)**-egenskap angitt til **ThisItem.AssignedTo**, som er et felt i **Aktiva**-datakilden. **[Text](controls/properties-core.md)**-egenskapen for de andre tre **[Etikett](controls/control-text-box.md)**-kontrollene i galleriet er angitt til lignende formler, og hver kontroll viser ulike felt i datakilden.  
...
| **ImageBackArrow1** |Når brukeren velger denne kontrollen, åpnes **BrowseScreen1**. |Angi **[OnSelect](controls/properties-core.md)**-egenskapen til denne formelen:<br>**Tilbake()** |
| **BildeSlett1** |Når brukeren velger denne kontrollen, slettes en post. |Angi **[OnSelect](controls/properties-core.md)**-egenskapen til denne formelen:<br>**Remove( Assets, BrowseGallery1.Selected )** |
...
**[DataField](controls/control-card.md)**-egenskapen for en **[Kort](controls/control-card.md)**-kontroll bestemmer hvilke felt kortet viser. I dette tilfellet er denne egenskapen angitt til **AssetID**. Kortet inneholder en **[Etikett](controls/control-text-box.md)**-kontroll der **[Text](controls/properties-core.md)**-egenskapen er angitt til **Parent.Default**. Denne kontrollen viser **Standard**-verdien for kortet, som er angitt via **[DataField](controls/control-card.md)**-egenskapen.
...
### <a name="editcreate-screen"></a>Rediger-/lagre-skjerm
...
| **ImageAccept1** |Når brukeren velger denne kontrollen, sendes endringene til datakilden. |Angi **[OnSelect](controls/properties-core.md)**-egenskapen til denne formelen:<br>**SubmitForm( EditForm1 )** |
...
## <a name="build-an-app-from-scratch"></a>Slik oppretter du en app fra grunnen av
...
## <a name="identify-test-data"></a>Identifisering av testdata
...
> [!NOTE]
> Hvis du bruker en SharePoint-liste eller en Excel-tabell som inneholder kolonnenavn med mellomrom som datakilde, erstatter PowerApps mellomrommene med **"\_x0020\_"**. **Kolonnenavn** i SharePoint eller Excel, vil for eksempel vises som **Column_x0020_Name** i PowerApps når det vises i dataoppsettet, eller brukes i en formel.

For å følge resten av dette emnet nøyaktig, kan du lage en SharePoint-liste med tittelen Ice Cream, som inneholder disse dataene:

![SharePoint-listen Ice Cream](./media/working-with-forms/sharepointlist-icecream.png)
...
> Apper for nettbrett er svært like, men du vil kanskje ha et annet [skjermoppsett](#screen-design) for å få fullt utbytte av den ekstra skjermplassen.
    ...
    Eksemplene i resten av emnet er basert på en datakilde med tittelen **Ice Cream**.

## <a name="browse-records"></a>Bla gjennom poster
...
2. Angi galleriets **[Items](controls/properties-core.md)**-egenskap til **Ice Cream**.
3. Angi **[Text](controls/properties-core.md)**-egenskapen for den første etiketten i galleriet til **ThisItem.Title** hvis den er angitt til noe annet.
...
* **[Item](controls/control-form-detail.md)**-egenskap.  Posten som du vil vise.  Denne egenskapen er ofte tilknyttet **Selected** egenskapen for **[Galleri](controls/control-gallery.md)**-kontrollen, slik at brukeren kan velge en post i **[Galleri](controls/control-gallery.md)**-kontrollen og deretter drille ned i denne posten.
...
* Angi **[Item](controls/control-form-detail.md)**-egenskapen for **[Visningsskjema](controls/control-form-detail.md)**-kontrollen til **Gallery1.Selected**.
    ...
    ![Visningsskjema for Ice Cream-datakilden koblet til gallerikontrollen](./media/working-with-forms/view-form-select-coconut.png)
...
![Visningsskjema for Ice Cream-datakilden](./media/working-with-forms/icecream-edit.png)
...
![Redigering av skjema for Ice Cream-datakilden](./media/working-with-forms/edit-icecream-save.png)
...
1. Legg til en ekstra **[Knapp](controls/control-button.md)**-kontroll, angi **[Text](controls/properties-core.md)**-egenskapen til å vise **Avbryt**, og angi **[OnSelect](controls/properties-core.md)**-egenskapen til denne formelen: <br>**ResetForm( Form1 ); Back()**
    ...
    Denne formelen forkaster eventuelle ulagrede endringer, og åpner det forrige skjermbildet.
   
    ![Visningsskjema for Ice Cream-datakilden](./media/working-with-forms/edit-icecream-cancel.png)
...
![Prøv ut ice cream-appen](./media/working-with-forms/try-icecream.png)

## <a name="create-a-record"></a>Oppretting av en post
...
Når skjemaet er i **Ny**-modus, er verdien for hvert felt angitt til standardverdiene for datakilden. Posten som er angitt i skjemaets **[Item](controls/control-form-detail.md)**-egenskap ignoreres.  
...
## <a name="delete-a-record"></a>Sletting av en post
...
## <a name="handling-errors"></a>Håndtering av feil
...
## <a name="refresh-data"></a>Oppdatering av data
...
1. Legg til en **[Knapp](controls/control-button.md)**-kontroll på skjermen med **[Galleri](controls/control-gallery.md)**-kontrollen, og angi **[Text](controls/properties-core.md)**-egenskapen til å vise **Refresh**.

2. Angi **[OnSelect](controls/properties-core.md)**-egenskapen for kontrollen til denne formelen:<br> **Refresh( 'Ice Cream' )**

    ![Oppdater datakilden](./media/working-with-forms/browse-icecream-refresh.png)

## <a name="search-and-sort-the-gallery"></a>Søk og sortering i galleriet
...
![Kontroller for sortering og søk på Bla gjennom-skjermen](./media/working-with-forms/afd-browse-search-sort.png)

Når brukeren velger Sorter-knappen, reverseres sorteringsrekkefølgen i galleriet. Hvis du vil opprette denne virkemåten, brukes en *kontekstvariabel* til å spore retningen som galleriet er sortert i. Når brukeren velger denne knappen, oppdateres variabelen, og retningen reverseres. **[OnSelect](controls/properties-core.md)**-egenskapen for sorter-knappen er angitt til denne formelen: **UpdateContext( {SortDescending1: !SortDescending1} )**
...
Hvis brukeren arbeider på den samme skjermen, må du passe på at brukeren ikke kan endre valget i **[Galleriet](controls/control-gallery.md)** og risikere å miste endringer i **[Redigeringsskjema](controls/control-form-detail.md)**-kontrollen.  Hvis du vil forhindre at brukeren skal velge en ny post før endringene i en annen post er lagret, kan du angi **[Disabled](controls/properties-core.md)**-egenskapen for galleriet til denne formelen:<br>
**EditForm.Unsaved**
